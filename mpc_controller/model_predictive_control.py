...
def plan_foot_contact_force(
    ...
    rpy_dot = np.clip(rpy_dot, -_MAX_ABS_ANGULAR_VELOCITY, _MAX_ABS_ANGULAR_VELOCITY)
    ...
    com_torque[2] = (
        -avg_bogy_height * _TORQUE_KD * (rpy_dot[2] - desired_com_angular_velocity[2])
    )
    ...
    com_force[2] += mass * 9.8 - _FORCE_KP * (avg_bogy_height - desired_com_position[2])
    ...
    all_contact_force = -np.matmul(
        np.linalg.pinv(foot_force_to_com), com_force_torque
    ).transpose()
    contact_force = {}
