...
_MPC_WEIGHTS = (5, 5, 0.2, 0, 0, 10, 0.0, 0.0, 1.0, 1.0, 1.0, 0.0, 0)
...
class TorqueStanceLegController(leg_controller.LegController):
    ...
    def get_action(self):
        ...
        predicted_contact_forces = self._cpp_mpc.compute_contact_forces(
            [0],  # com_position
            np.asarray(
                self._state_estimator.com_velocity_body_frame, dtype=np.float64
            ),  # com_velocity
            ...
            np.asarray(
                self._robot.GetBaseRollPitchYawRate(), dtype=np.float64
            ),  # com_angular_velocity
            foot_contact_state,  # foot_contact_states
            np.array(
                self._robot.GetFootPositionsInBaseFrame().flatten(), dtype=np.float64
