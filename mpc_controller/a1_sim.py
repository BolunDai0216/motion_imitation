...
INIT_MOTOR_ANGLES = np.array(
    [A1_DEFAULT_ABDUCTION_ANGLE, A1_DEFAULT_HIP_ANGLE, A1_DEFAULT_KNEE_ANGLE] * NUM_LEGS
)
MOTOR_NAMES = [
    "FR_hip_joint",
    "FR_upper_joint",
...
class A1MotorModel(object):
    ...
    def __init__(
        self, kp, kd, torque_limits=None, motor_control_mode=MOTOR_CONTROL_POSITION
    ):
        self._kp = kp
        self._kd = kd
...
class SimpleRobot(object):
    def __init__(self, pybullet_client, robot_uid, simulation_time_step):
        ...
        self._motor_model = A1MotorModel(
    ...
    def _SettleDownForReset(self, reset_time):
        ...
        for _ in range(500):
            self._StepInternal(
                INIT_MOTOR_ANGLES, motor_control_mode=MOTOR_CONTROL_POSITION
            )
    ...
    def MapContactForceToJointTorques(self, leg_id, contact_force):
        ...
        com_dof = 6
        for joint_id in range(leg_id * motors_per_leg, (leg_id + 1) * motors_per_leg):
            motor_torques[joint_id] = (
                all_motor_torques[com_dof + joint_id] * self._motor_direction[joint_id]
            )
    ...
    def joint_angles_from_link_position(
        ...
        if not position_in_world_frame:
            ...
            (
                ...
                self.quadruped
            )  # robot.GetBasePosition(), robot.GetBaseOrientation()
    ...
    def _EndEffectorIK(self, leg_id, position, position_in_world_frame):
        ...
        joint_position_idxs = [
            i
            for i in range(
                leg_id * motors_per_leg, leg_id * motors_per_leg + motors_per_leg
            )
        ]
    ...
    def GetTrueBaseOrientation(self):
        pos, orn = self.pybullet_client.getBasePositionAndOrientation(self.quadruped)
        return orn
    ...
    def GetFootPositionsInBaseFrame(self):
        ...
        for foot_id in self.GetFootLinkIDs():
            foot_positions.append(self.link_position_in_base_frame(link_id=foot_id))
